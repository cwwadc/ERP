...
public class AccountService {
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int updateAccount(Account account, HttpServletRequest request)throws Exception {
        int result=0;
        try{
            result = accountMapper.updateByPrimaryKeySelective(account);
            logService.insertLog("账户",
                    new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_EDIT).append(account.getId()).toString(), request);
        }catch(Exception e){
            JshException.writeFail(logger, e);
        }
        return result;
