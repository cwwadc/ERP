...
public class DepotHeadController {
    ...
    public String getList(@RequestParam(value = Constants.PAGE_SIZE, required = false) Integer pageSize,
        ...
        String searchParam = parameterMap.get(Constants.SEARCH);
        String OrganId = StringUtil.getInfo(searchParam, "OrganId");
        String type = StringUtil.getInfo(searchParam, "type");
        String subType = StringUtil.getInfo(searchParam, "subType");
        String number = StringUtil.getInfo(searchParam, "number");
        String beginTime = StringUtil.getInfo(searchParam, "beginTime");
        String endTime = StringUtil.getInfo(searchParam, "endTime");
        String materialParam = StringUtil.getInfo(searchParam, "materialParam");
        String depotIds = StringUtil.getInfo(searchParam, "depotIds");
        List<DepotHeadVo4List> dataList = depotHeadService.selectCaigouList(OrganId, type, subType, number, beginTime, endTime, materialParam, depotIds,Integer.parseInt(offset) , pageSize);
     ...
     * @return
     */
    @GetMapping(value = "/exportOrderExcel",produces="application/json")
    public BaseResponseInfo exportExcel(
        ...
        try {
            List<DepotHeadVo4List> dataList = depotHeadService.selectCaigouList(OrganId, type, subType, number, beginTime, endTime, materialParam, depotIds,Integer.parseInt(offset) , pageSize);
            //存放数据json数组
            String[] names = {"供应商名称", "单据编号", "商品信息", "单据日期", "操作员", "金额合计", "状态"};
            String title = "采购订单报表";
            List<String[]> objects = new ArrayList<String[]>();
            if (dataList != null && dataList.size() > 0) {
                for (DepotHeadVo4List headEx : dataList) {
                    String[] objs = new String[7];
                    objs[0] = headEx.getOrganName();
                    objs[1] = headEx.getNumber();
                    objs[2] = headEx.getMaterialsList();
                    objs[3] = headEx.getOpertimeStr();
                    objs[4] = headEx.getOperpersonname();
                    objs[5] = headEx.getTotalprice().toString();
                    if ("0".equals(headEx.getStatus())) {
                        objs[6] = "未审核";
                    } else if ("1".equals(headEx.getStatus())) {
                        objs[6] = "已审核";
                    } else {
                        objs[6] = "已转采购或销售";
                    }
     ...
     * @return
     */
    @GetMapping(value = "/exportTuiHuoExcel",produces="application/json")
    public BaseResponseInfo exportTuiHuoExcel(
            @RequestParam(value = Constants.PAGE_SIZE, required = false) Integer pageSize,
            @RequestParam(value = Constants.CURRENT_PAGE, required = false) Integer currentPage,
            @RequestParam(value = "OrganId", required = false) String OrganId,
            @RequestParam(value = "type", required = false) String type,
        ...
        try {
            List<DepotHeadVo4List> dataList = depotHeadService.selectCaigouList(OrganId, type, subType, number, beginTime, endTime, materialParam, depotIds,Integer.parseInt(offset) , pageSize);
            //存放数据json数组
            String[] names = {"供应商名称", "单据编号", "商品信息", "单据日期", "操作员", "金额合计", "含税合计", "优惠后金额", "收款"};
            String title = "采购订单报表";
            List<String[]> objects = new ArrayList<String[]>();
            if (dataList != null && dataList.size() > 0) {
                for (DepotHeadVo4List headEx : dataList) {
                    ...
                    objs[3] = headEx.getOpertimeStr();
                    objs[4] = headEx.getOperpersonname();
                    objs[5] = headEx.getTotalprice().toString(); // 金额合计
                    objs[6] = headEx.getDiscountmoney().add(headEx.getDiscountlastmoney()).toString(); //含税合计
                    objs[7] = headEx.getDiscountlastmoney().toString(); // 优惠后金额
                    objs[8] = headEx.getChangeamount().toString(); // 收款
                    objects.add(objs);
                }
            }
            File file = ExcelUtils.exportObjectsWithoutTitle(title, names, title, objects);
