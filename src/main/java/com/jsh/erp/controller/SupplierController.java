...
public class SupplierController {
     ...
     * @return
     */
    @PostMapping(value = "/findBySelect_all")
    public JSONArray findBySelectAll(HttpServletRequest request)throws Exception {
        JSONArray arr = new JSONArray();
        try {
            ...
            if (null != supplierList) {
                for (Supplier supplier : supplierList) {
                    ...
                    item.put("id", supplier.getId());
                    //客户名称
                    item.put("introducer", supplier.getSupplier());
                    dataArray.add(item);
                }
            }
            arr = dataArray;
        ...
        try {
            ...
            if (null != dataList) {
                for (Supplier supplier : dataList) {
                     ...
                     * 20200222新增字段
                     */
                    item.put("province", supplier.getProvince());
                    item.put("logistics", supplier.getLogistics());
                    item.put("nature", supplier.getNature());
                    item.put("introducer", supplier.getIntroducer());
    ...
    public void exportExcel(@RequestParam("province") String province,
        ...
        try {
            List<Supplier> dataList = supplierService.findByAll(province, nature, supplier, type, phonenum, telephone, description);
            String[] names = {
                    "名称", "类型", "客户组", "性质", "联系人", "电话", "电子邮箱", "预收款", "期初应收", "期初应付",
                    "备注", "传真", "物流", "介绍人", "手机", "地址", "纳税人识别号", "开户行", "账号", "税率", "状态"
            };
            String title = "信息报表";
            List<String[]> objects = new ArrayList<String[]>();
            if (null != dataList) {
                for (Supplier s : dataList) {
                    String[] objs = new String[21];
                    objs[0] = s.getSupplier();
                    objs[1] = s.getType();
                    objs[2] = s.getProvince();
                    objs[3] = s.getNature();
                    objs[4] = s.getContacts();
                    objs[5] = s.getPhonenum();
                    objs[6] = s.getEmail();
                    objs[7] = s.getAdvancein() == null? "" : s.getAdvancein().toString();
                    objs[8] = s.getBeginneedget() == null? "" : s.getBeginneedget().toString();
                    objs[9] = s.getBeginneedpay() == null? "" : s.getBeginneedpay().toString();
                    objs[10] = s.getDescription();
                    objs[11] = s.getFax();
                    objs[12] = s.getLogistics();
                    objs[13] = s.getIntroducer();
                    objs[14] = s.getTelephone();
                    objs[15] = s.getAddress();
                    objs[16] = s.getTaxnum();
                    objs[17] = s.getBankname();
                    objs[18] = s.getAccountnumber();
                    objs[19] = s.getTaxrate() == null? "" : s.getTaxrate().toString();
                    objs[20] = s.getEnabled() ? "启用" : "禁用";
                    objects.add(objs);
                }
            }
    ...
    public String importFun(MultipartFile supplierFile)throws Exception{
        ...
        try {
            ...
            //每行中数据顺序 "名称","类型","客户组","性质","联系人","电话","电子邮箱","预收款","期初应收","期初应付","备注","传真","物流","介绍人","手机","地址","纳税人识别号","开户行","账号","税率","状态"
            ...
            for (int i = 1; i < src.getRows(); i++) {
                ...
                s.setProvince(ExcelUtils.getContent(src, i, 2));
                s.setNature(ExcelUtils.getContent(src, i, 3));
                s.setContacts(ExcelUtils.getContent(src, i, 4));
                s.setPhonenum(ExcelUtils.getContent(src, i, 5));
                s.setEmail(ExcelUtils.getContent(src, i, 6));
                s.setAdvancein(parseBigDecimalEx(ExcelUtils.getContent(src, i, 7)));
                s.setBeginneedget(parseBigDecimalEx(ExcelUtils.getContent(src, i, 8)));
                s.setBeginneedpay(parseBigDecimalEx(ExcelUtils.getContent(src, i, 9)));
                s.setDescription(ExcelUtils.getContent(src, i, 10));
                s.setFax(ExcelUtils.getContent(src, i, 11));
                s.setLogistics(ExcelUtils.getContent(src, i, 12));
                s.setIntroducer(ExcelUtils.getContent(src, i, 13));
                s.setTelephone(ExcelUtils.getContent(src, i, 14));
                s.setAddress(ExcelUtils.getContent(src, i, 15));
                s.setTaxnum(ExcelUtils.getContent(src, i, 16));
                s.setBankname(ExcelUtils.getContent(src, i, 17));
                s.setAccountnumber(ExcelUtils.getContent(src, i, 18));
                s.setTaxrate(parseBigDecimalEx(ExcelUtils.getContent(src, i, 19)));
