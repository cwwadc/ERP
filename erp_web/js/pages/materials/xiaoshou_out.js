	...
	function statisticsFun(body,UnitPrice,OperNumber,footer,taxRate){
		...
		// var total = 0;
		// body.find("[field='SaleRoyalty']").each(function(){
		// 	if($(this).find("div").text()!==""){
		// 		total = total + parseFloat($(this).find("div").text().toString());
		// 	}
		// });
		// if (royaltyMethod){
		// 	// 百分比
		...
		// 	total = retailRoyaltyTotal;
		// 	footer.find("[field='SaleRoyalty']").find("div").text((total).toFixed(2)); //价税合计的页脚总计
		// }
		// if(listSubType == "销售" && listType == "出库"){
		// 	total = wholesaleRoyaltyTotal;
		// 	footer.find("[field='SaleRoyalty']").find("div").text((total).toFixed(2)); //价税合计的页脚总计
		// }
	}
	//初始化表格数据-商品列表-编辑状态
	...
	//初始化表格数据-商品列表-查看状态
	function initTableData_material_show2(TotalPrice){
		var isShowAnotherDepot = true; //显示对方仓库,true为隐藏,false为显示
		var anotherDepotHeadName = ""; //对方仓库的列的标题
		...
		$('#materialDataShow2').datagrid({
			...
			columns:[[
				...
				{ title: '含税单价',field: 'TaxUnitPrice',editor:'validattebox',hidden:isShowTaxColumn,width:85},
				...
				{ title: '价税合计',field: 'TaxLastMoney',editor:'validatebox',hidden:isShowTaxColumn,width:95},
	...
	function showDepotHead(index){
		...
		initTableData_material_show2(TotalPrice); //商品列表-查看状态
