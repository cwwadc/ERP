	...
	//初始化销售人员(零售出库)
	function initSalesman_flag(){
		// $('#Salesman_flag').combobox({
		// 	url: "/person/getPersonByNumType?type=1",
		// 	valueField:'id',
		// 	textField:'name',
		// 	multiple: true
		...
		$.ajax({
			...
			success: function(data) {
				for(var i = 0; i< data.length; i++){
					$("#Salesman_flag").append("<option value="+data[i].id+">"+data[i].name+"</option>");
				}
			}
		});
	...
	function initTableData_material(type,TotalPrice){
		...
		var isShowSellStockOutTypeColumn = true; // 零售出库和销售出库标识 true 隐藏  false 显示
		if((listSubType == "零售" || listSubType == "销售") && (listType == "出库")){
			isShowSellStockOutTypeColumn = false; //显示
		}
		...
		$('#materialData').datagrid({
			...
			columns:[[
				...
				{ title: '提成方式',field: 'royaltyMethod',editor:'validatebox',hidden:true,width:60},
				{ title: '批发提成',field: 'wholesaleRoyalty',editor:'validatebox',hidden:true,width:60},
				{ title: '零售提成',field: 'retailRoyalty',editor:'validatebox',hidden:true,width:60},
				{ title: '销售人员提成',field: 'SaleRoyalty',editor:'validatebox',hidden:isShowSellStockOutTypeColumn, width:100},
				{ title: '运输方式',field: 'Transport',editor:'validatebox',hidden:isShowSellStockOutTypeColumn, width:100},
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if(res && res.code === 200 && res.data && res.data[0]) {
                                ...
                                body.find("[field='Unit']").find(input).off("click").on("click",function(){
                                    if(basicUnit && otherUnit) {
                                        ...
                                        $('.unit-list ul li').off("click").on("click",function(){
											...
											//销售提成的合计
											var royaltyMethod = res.data[0].royaltyMethod; // 提成方式 0固定提成 1百分比提成
											body.find("[field='royaltyMethod']").find(input).val(royaltyMethod);
											var wholesaleRoyalty = res.data[0].wholesaleRoyalty;// 批发提成
											body.find("[field='wholesaleRoyalty']").find(input).val(wholesaleRoyalty);
											var retailRoyalty = res.data[0].retailRoyalty;//零售提成
											body.find("[field='retailRoyalty']").find(input).val(retailRoyalty);
											var wholesaleRoyaltyTotal = 0; //批发合计
											var retailRoyaltyTotal = 0; //零售合计
											if (royaltyMethod){
												// 百分比
												wholesaleRoyaltyTotal = wholesaleRoyaltyTotal + UnitPrice*OperNumber*wholesaleRoyalty/100; //批发
												retailRoyaltyTotal = retailRoyaltyTotal +  UnitPrice*OperNumber*retailRoyalty/100; //零售
											}else{
												// 固定提成
												wholesaleRoyaltyTotal = wholesaleRoyaltyTotal + OperNumber*wholesaleRoyalty; //批发
												retailRoyaltyTotal = retailRoyaltyTotal +  OperNumber*retailRoyalty;//零售
											}
											if(listSubType == "零售" && listType == "出库"){
												body.find("[field='SaleRoyalty']").find(input).val(retailRoyaltyTotal);
											}
											if(listSubType == "销售" && listType == "出库"){
												body.find("[field='SaleRoyalty']").find(input).val(wholesaleRoyaltyTotal);
											}
							...
							if (royaltyMethod){
								// 百分比
								wholesaleRoyaltyTotal = wholesaleRoyaltyTotal + detailPrice*1*wholesaleRoyalty/100; //批发
								retailRoyaltyTotal = retailRoyaltyTotal +  detailPrice*1*retailRoyalty/100; //零售
							}else{
								// 固定提成
								wholesaleRoyaltyTotal = wholesaleRoyaltyTotal + 1*wholesaleRoyalty; //批发
								retailRoyaltyTotal = retailRoyaltyTotal +  1*retailRoyalty;//零售
	...
	function initTableData_material_show(TotalPrice){
		...
		$('#materialDataShow2').datagrid({
			...
			columns:[[
				...
				{ title: '品名(型号)(扩展信息)(单位)',field: 'MaterialName',width:250},
				...
				{ title: '含税单价',field: 'TaxUnitPrice',editor:'validattebox',hidden:false,width:85},
				{ title: '金额',field: 'AllPrice',editor:'validatebox',width:85},
				{ title: '大写金额',field: 'rmbCapitalization',editor:'validatebox',hidden:true,width:85},
				{ title: '提成方式',field: 'royaltyMethod',editor:'validatebox',hidden:true},
				{ title: '批发提成',field: 'wholesaleRoyalty',editor:'validatebox',hidden:true},
				{ title: '零售提成',field: 'retailRoyalty',editor:'validatebox',hidden:true},
				{ title: '运输方式',field: 'Transport',editor:'validatebox',hidden:isShowSellStockOutTypeColumn, width:85},
				{ title: '销售人员提成',field: 'SaleRoyalty',editor:'validatebox',hidden:isShowSellStockOutTypeColumn, width:95},
				{ title: '价税合计',field: 'TaxLastMoney',editor:'validatebox',hidden:false,width:95},
				{ title: '备注',field: 'Remark',editor:'validatebox',width:173},
				{ title: '品名-别',field: 'OtherField1',editor:'validatebox',hidden:otherColumns,width:60},
				{ title: '型号-别',field: 'OtherField2',editor:'validatebox',hidden:otherColumns,width:60},
				{ title: '颜色-别',field: 'OtherField3',editor:'validatebox',hidden:otherColumns,width:60},
				{ title: '备注1',field: 'OtherField4',editor:'validatebox',hidden:true,width:60},
		...
		$.ajax({
			...
			success: function (res) {
				if(res && res.code === 200) {
					...
					var rmbCapitalization = res.data.rows[0].rmbCapitalization;
					var OperNumber = res.data.rows[0].OperNumber;
					var DiscountMoney = $("#DiscountMoneyShow").text() - 0; //优惠金额
					var DiscountLastMoney = $("#DiscountLastMoneyShow").text() - 0; //优惠后金额
					var DiscountTotal = DiscountMoney + DiscountLastMoney;
					var array = [];
					array.push({
						"MaterialName": "合计金额：" + rmbCapitalization,
						"OperNumber": OperNumber,
						"AllPrice": "合计：￥"+ AllPrice
						// "TaxLastMoney": "￥"+ DiscountTotal
					});
					data.footer = array;
					$("#materialDataShow2").datagrid('loadData', data);
				}
			},
			error:function() {
				$.messager.alert('查询提示','查询数据后台异常，请稍后再试！','error');
	...
	function showDepotHead(index){
		...
		$("#TransportShow").text(res.transport); // 运输方式
		$("#phoneShow").text(res.phonenum); // 客户电话
		$("#addressShow").text(res.address); // 客户地址
	    $("#NumberShow").text(res.number); // 单据编号
	    $("#OperTimeShow").text(res.opertimeStr); // 销售日期
	    $('#OrganIdShow').text(res.organName); // 客户名称
	    ...
	    $("#ChangeAmountShow").text(res.changeamount); // 本次收款
	    $("#RemarkShow").text(res.remark); // 单据备注
		$("#DiscountShow").text(0); // 优惠率
		$("#DiscountMoneyShow").text(0); // 收款优惠
		$("#DiscountLastMoneyShow").text(res.changeamount); // 优惠后金额
		$("#DebtShow").text(0); // 本次欠款
		...
		if(listSubType == "零售" &&(listType == "出库" ||  listSubType == "退货")){
			var arr = $("#Salesman_flag").val().split(",");
			var salesmanStr = "";
			for(var i=0;i<arr.length;i++){
				if(arr[i]){
					if(i === arr.length-1){
		...
		$("#saveDepotHead").off("click").on("click",function(){
			if(!$('#depotHeadFM').form('validate')){
                ...
                if(listTitle === "采购订单列表"){
					...
					if(!$('#Salesman_flag').val()){
						$.messager.alert('提示','请选择销售人员！','warning');
						return;
					}
					if(!$('#AccountId').val()){
	...
	function autoReckon() {
	    ...
	    setTimeout(function(){
			...
			body.find("[field='OperNumber']").find(input).off("keyup").on("keyup",function(){
				...
				body.find("[field='TaxLastMoney']").find(input).val((UnitPrice*OperNumber*(1+taxRate/100)).toFixed(2)); //价税合计
				//销售提成的合计
				var royaltyMethod = body.find("[field='royaltyMethod']").find(input).val(); // 提成方式 0固定提成 1百分比提成
				var wholesaleRoyalty = body.find("[field='wholesaleRoyalty']").find(input).val(); // 批发提成
				var retailRoyalty =  body.find("[field='retailRoyalty']").find(input).val(); //零售提成
				var wholesaleRoyaltyTotal = 0; //批发合计
				var retailRoyaltyTotal = 0; //零售合计
				if (royaltyMethod){
					// 百分比
			...
			//修改销售人员提成，自动计算单价、税额、价税合计和合计
			body.find("[field='SaleRoyalty']").find(input).off("keyup").on("keyup",function(){
				var OperNumber = body.find("[field='OperNumber']").find(input).val(); //数量
				var taxRate = body.find("[field='TaxRate']").find(input).val(); //税率
				var AllPrice =$(this).val()-0; //金额
				var UnitPrice = (AllPrice/OperNumber).toFixed(2);
