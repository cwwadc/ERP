...
function showDepotHeadDetails(pageNo, pageSize) {
    var OrganId = $('#searchOrganId').combobox('getValue');
...
function exportExcel() {
	showDepotHeadExcelEachDetails(1, 3000);
}

function showDepotHeadExcelEachDetails(pageNo, pageSize) {
	var OrganId = $('#searchOrganId').combobox('getValue');
	var state = $.trim($("#searchState").val());
	var number = $.trim($("#searchNumber").val())
	var materialParam = $.trim($("#searchMaterial").val());
	var beginTime = $.trim($("#searchBeginTime").val());
	var endTime = $.trim($("#searchEndTime").val());
	if (beginTime) {
	...
	$.ajax({
		...
		dataType: "json",
		data: ({
			search: JSON.stringify(headData),
			currentPage: pageNo,
			pageSize: pageSize
		}),
		success: function (res) {
			if (res && res.code === 200 && res.data) {
				var HeadIds = res.data.page.total;
				if (HeadIds > 0) {
					if (pageSize === 3000) {
						window.location.href = "/depotHead/exportTuiHuoExcel?browserType=" + getOs()
							+ "&pageSize=" + pageSize
							+ "&currentPage=" + pageNo
							+ "&OrganId=" + OrganId
							+ "&type=" + listType
						...
						//获取排序后的产品ID
						$("#tableData").datagrid('loadData', res.data.page.rows);
					}
				} else {
