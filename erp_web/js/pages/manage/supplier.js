    ...
    $(function() {
        ...
        ininPager();
        bindEvent();
        initCustomergroup(); //初始化客户组
        initCustomergroup2(); //初始化客户组
        initCustomer(); //初始化客户（介绍人使用）
    });
    ...
    function initTableData() {
        ...
        $('#tableData').datagrid({
            ...
            columns:[[
                ...
                { title: '名称',field: 'supplier',width:100},
                { title: '客户组', field: 'province',width:50,align:"center"},
                { title: '性质', field: 'nature',width:50,align:"center"},
                ...
                { title: '物流', field: 'logistics',width:100,align:"center"},
                { title: '介绍人', field: 'introducer',width:100,align:"center"},
    ...
    function setOutputFun(){
        var province = $.trim($("#searchProvince").val());
        var nature = $.trim($("#searchNature").val());
        ...
        window.location.href = "/supplier/exportExcel?browserType=" + getOs()
            + "&province=" + province + "&nature=" + nature + "&supplier=" + supplier + "&type=" + listType +
            "&phonenum=" + phonenum + "&telephone=" + telephone + "&description=" + description;
    }
    ...
    function bindEvent(){
        ...
        $("#saveSupplier").off("click").on("click", function () {
            ...
            if(!$('#province').combobox('getValue')){
                $.messager.alert('提示','请选择客户组！','warning');
                return;
            }
            if(listType == "客户"){
                if($('#nature').val()==null || $('#nature').val()==''){
                    $.messager.alert('提示','请选择客户性质！','warning');
                    return;
                }
            }
    ...
    function initCustomergroup(){
        $('#province').combobox({
            url: "/customergroup/findBySelect_sup",
            valueField:'province',
            textField:'province',
            ...
            onLoadSuccess: function(res) {
                ...
                for(var i = 0; i< data.length; i++){
                    $("#province").combobox("setValue", data[i].province);
                    $("#province").combobox("setText", data[i].province);
                }
            },
    ...
    //初始化客户组
    function initCustomergroup2(){
        $.ajax({
            url: "/customergroup/findBySelect_sup",
            type: 'POST',
            success: function(data) {
                for(var i = 0; i< data.length; i++){
                    $("#searchProvince").append("<option value="+data[i].province+">"+data[i].province+"</option>");
                }
            }
        });
    }
    ...
    function initCustomer(){
        $('#introducer').combobox({
            url: "/supplier/findBySelect_all",
            valueField:'introducer',
            textField:'introducer',
            filter: function(q, row){
                var opts = $(this).combobox('options');
                return row[opts.textField].indexOf(q) >-1;
            },
            onLoadSuccess: function(res) {
                ...
                var data = $(this).combobox('getData');
                for(var i = 0; i< data.length; i++){
                    $("#introducer").combobox("setValue", data[i].introducer);
                    $("#introducer").combobox("setText", data[i].introducer);
                }
            },
            onSelect: function(rec){
