        ...
        try {
            ...
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    ...
                    if (!StringUtil.isEmpty(diEx.getAllprice().toString())){
                        String rmbCapitalization = RMBChangeUtils.rmbChange(diEx.getAllprice().toString());
                        item.put("rmbCapitalization",rmbCapitalization);
                    }
                    item.put("Transport", diEx.getTransport());
                    ...
                    item.put("op", 1);
                    // 计算提成合计
                    Boolean royaltyMethod = diEx.getRoyaltyMethod(); // 提成方式 0固定提成 1百分比提成
                    item.put("royaltyMethod",royaltyMethod);
                    BigDecimal wholesaleRoyalty = diEx.getWholesaleRoyalty(); // 批发提成
                    item.put("wholesaleRoyalty",wholesaleRoyalty);
                    BigDecimal retailRoyalty = diEx.getRetailRoyalty();//零售提成
                    item.put("retailRoyalty", retailRoyalty);
//                    BigDecimal wholesaleRoyaltyTotal; // 批发提成合计
//                    BigDecimal retailRoyaltyTotal; // 零售提成合计
//                    if (royaltyMethod){
//                        // true 为百分比提成
//                        wholesaleRoyaltyTotal = diEx.getUnitprice().multiply(wholesaleRoyalty).multiply(diEx.getOpernumber()).divide(new BigDecimal(100));
//                        retailRoyaltyTotal = diEx.getUnitprice().multiply(retailRoyalty).multiply(diEx.getOpernumber()).divide(new BigDecimal(100));
//                    }else{
//                        // false 为固定提成
//                        wholesaleRoyaltyTotal =wholesaleRoyalty.multiply(diEx.getOpernumber());
//                        retailRoyaltyTotal = retailRoyalty.multiply(diEx.getOpernumber());
//                    }
//                    item.put("wholesaleRoyaltyTotal", wholesaleRoyaltyTotal);
//                    item.put("retailRoyaltyTotal", retailRoyaltyTotal);
                    item.put("SaleRoyalty",diEx.getSaleRoyalty());
                    dataArray.add(item);
                }
            }
