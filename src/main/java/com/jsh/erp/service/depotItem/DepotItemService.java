...
public class DepotItemService {
     ...
     * */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public String saveDetials(String inserted, String deleted, String updated, Long headerId, Long tenantId, HttpServletRequest request, DepotHead depotHeadVo) throws Exception{
        logService.insertLog("单据明细",
...
//                            String UnitName = findUnitName(mId); //查询计量单位名称
                    ...
                    BigDecimal wholesaleRoyalty = new BigDecimal(0);// 批发
                    BigDecimal retailRoyalty = new BigDecimal(0);// 零售
                    BigDecimal total = new BigDecimal(0); // 合计
                    if (("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())) || ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType()))){
                        if (!StringUtil.isEmpty(tempInsertedJson.get("wholesaleRoyalty").toString())){
                            wholesaleRoyalty = tempInsertedJson.getBigDecimal("wholesaleRoyalty");
                        }
                        if (!StringUtil.isEmpty(tempInsertedJson.get("retailRoyalty").toString())){
                            retailRoyalty = tempInsertedJson.getBigDecimal("retailRoyalty");
                        }
                        if (!StringUtil.isEmpty(tempInsertedJson.get("royaltyMethod").toString())){
                            Boolean royaltyMethod = tempInsertedJson.getBoolean("royaltyMethod");
                            if (royaltyMethod){
                                // 百分比提成
                                if ("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempInsertedJson.getBigDecimal("UnitPrice")
                                            .multiply(tempInsertedJson.getBigDecimal("OperNumber"))
                                            .multiply(retailRoyalty).divide(new BigDecimal(100));
                                }else if ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempInsertedJson.getBigDecimal("UnitPrice")
                                            .multiply(tempInsertedJson.getBigDecimal("OperNumber"))
                                            .multiply(wholesaleRoyalty).divide(new BigDecimal(100));
                                }
                            }else {
                                // 固定提成
                                if ("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempInsertedJson.getBigDecimal("OperNumber").multiply(retailRoyalty);
                                }else if ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempInsertedJson.getBigDecimal("OperNumber").multiply(wholesaleRoyalty);
                                }
                            }
                        }
                    }
                    depotItem.setSaleRoyalty(total);
//                    if (!StringUtil.isEmpty(tempInsertedJson.get("SaleRoyalty").toString())){
//                        // 添加销售提成
//                        depotItem.setSaleRoyalty(total);
//                    }
                    // 添加运输方式
                    depotItem.setTransport(tempInsertedJson.getString("Transport"));
...
//                            String UnitName = findUnitName(mId); //查询计量单位名称
                    ...
                    BigDecimal total = new BigDecimal(0); // 合计
                    if (("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())) || ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType()))){
                        if (!StringUtil.isEmpty(tempUpdatedJson.get("wholesaleRoyalty").toString())){
                            wholesaleRoyalty = tempUpdatedJson.getBigDecimal("wholesaleRoyalty");
                        }
                        if (!StringUtil.isEmpty(tempUpdatedJson.get("retailRoyalty").toString())){
                            retailRoyalty = tempUpdatedJson.getBigDecimal("retailRoyalty");
                        }
                        if (!StringUtil.isEmpty(tempUpdatedJson.get("royaltyMethod").toString())){
                            Boolean royaltyMethod = tempUpdatedJson.getBoolean("royaltyMethod");
                            if (royaltyMethod){
                                // 百分比提成
                                if ("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempUpdatedJson.getBigDecimal("UnitPrice")
                                            .multiply(tempUpdatedJson.getBigDecimal("OperNumber"))
                                            .multiply(retailRoyalty).divide(new BigDecimal(100));
                                }else if ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempUpdatedJson.getBigDecimal("UnitPrice")
                                            .multiply(tempUpdatedJson.getBigDecimal("OperNumber"))
                                ...
                                // 固定提成
                                if ("零售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempUpdatedJson.getBigDecimal("OperNumber").multiply(retailRoyalty);
                                }else if ("销售".equals(depotHeadVo.getSubtype()) && "出库".equals(depotHeadVo.getType())){
                                    total = tempUpdatedJson.getBigDecimal("OperNumber").multiply(wholesaleRoyalty);
                                }
                            }
                        }
                    }
...
//                    }
                    // 添加运输方式
                    depotItem.setTransport(tempUpdatedJson.getString("Transport"));
